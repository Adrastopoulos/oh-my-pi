...
- Fixed speculative compaction re-sending a native (server-side) compaction that failed for a reason a retry would hit again, such as an exhausted output budget; the failure now holds for the session and model until a compaction commits, and the threshold pass falls back to the next method instead of waiting on repeated full-context requests ([#13310](https://github.com/can1357/oh-my-pi/pull/13310) by [@alphastorm](https://github.com/alphastorm))
